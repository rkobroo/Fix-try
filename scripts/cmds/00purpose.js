...
module.exports = {
    ...
    onStart: async function ({ message, args, event, api }) {
        ...
        if (mention.length == 0) return message.reply("Please mention someone");
            ...
            bal(one, two).then(ptth => { message.reply({ body: "🥺Please Babe Accept My 😘 </3", attachment: fs.createReadStream(ptth) }) })
        } else {
            const one = mention[1], two = mention[0];
            bal(one, two).then(ptth => { message.reply({ body: "🥺Please Babe Accept My Love ❤️</3", attachment: fs.createReadStream(ptth) }) })
        }
    }
};
